...
- **AI 老师**：点右下角浮动按钮开启与 AI 老师的实时语音对话（Gemini Live）。老师能带着学当前单词卡（介绍发音/意思/用法，引导跟读单词和例句并鼓励式纠音）、给当前单词组做听写（学生说"可以了"才读下一个）、或用简单英语自由聊天（老师会主动抛话题提问，孩子答不上来时先示范一句怎么答、再让孩子试一次）——三种玩法开口说即可切换，无需任何按钮。对话面板里还能拍照给老师看：老师默默记住照片，等孩子开口问（"Look at this!"）再一起聊照片、顺带教几个单词。切换单词组/单词卡时上下文自动同步给老师，问"介绍一下这个单词"她就知道你看的是哪张卡。对话带双向实时字幕；设置页（首页右上角 ⚙️）可自定义老师性格、音色与"听说灵敏度"（老师判定孩子开口/说完的快慢，跟读短单词没反应或总被抢话时微调）。
...
go run ./cmd/spike -teacher    # AI 老师链路独立验证（上下文注入 + 对话轮 + 发照片 + 转写，需 GEMINI_API_KEY）
