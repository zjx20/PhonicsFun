...
7. **Live 会话复用**：每会话最多 8 词、13 分钟，收到 GoAway 或出错即轮换（常量在 `internal/llm/live.go`）；音频会话官方上限约 15 分钟，轮换阈值必须留余量。单词重生成走全新小会话。每词 2 轮且顺序固定：先整词（blend 收尾依赖 word.wav），后 blend 主体。
